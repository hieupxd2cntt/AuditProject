...
{
    public partial class ucTransact : ucModule,
        ...
        ICommonFieldSupportedModule
    {
        ...
        public override void InitializeLayout()
        {
            ...
            mainLayoutGroup.AppearanceItemCaption.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Far;                        
            CreateGroupLayoutPOS();                        
        }

        private void CreateGroupLayoutPOS()
        {
            mainLayout.BeginUpdate();
            try
            {                
                mainLayout.Root.GroupBordersVisible = false;
                LayoutControlGroup groupPOS = new LayoutControlGroup();
                groupPOS.Name = "GroupPOS";
                groupPOS.Text = "POS";
                mainLayout.Root.Add(groupPOS);

                List<LayoutControlItem> lstGridView = new List<LayoutControlItem>();
                foreach (var item in mainLayout.Root.Items)
                {
                    if (item is LayoutControlItem)
                    {
                        LayoutControlItem itemgroup = ((LayoutControlItem)item);
                        if (itemgroup.Control is DevExpress.XtraGrid.GridControl)
                        {
                            lstGridView.Add(itemgroup);
                        }                        
                    }
                }

                foreach (var itemgroup in lstGridView)
                {
                    ProcessLayoutControlItem(itemgroup, groupPOS);
                }
                // hardcode UI POSMAP here
                lstGridView[1].Move(lstGridView[0], InsertType.Right);
            ...
            cliensizeResize = mainLayout.Size;
        }

        private void ProcessLayoutControlItem(LayoutControlItem item, LayoutControlGroup grouppos)
        {            
            grouppos.Add(item);            
        }
                                ...
                                switch (ModuleInfo.SubModule)
                                {
                                    case Core.CODES.DEFMOD.SUBMOD.MAINTAIN_ADD:
                                            ...
                                            else
                                            {
                                                ...
                                                // TuanLM test thử luồng client đẩy vô sqlite
                                                //AppClient.Controllers.BDSController clientCon = new Controllers.BDSController();
                                                //clientCon.ExecuteMaintain(ModuleInfo.ModuleID, ModuleInfo.SubModule, values);
                                                // End test
