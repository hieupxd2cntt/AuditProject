    ...
    public class ModuleController : BaseController
    {
        ...
        public async Task<string> ExcuteStore2DataTable(ParramModuleQuery query)
        {
            ...
            try
            {
                ...
                cm.GetNpgsqlParameterValues(param, query.Fields);
                var checkParram = param.Where(x => x.NpgsqlDbType != NpgsqlTypes.NpgsqlDbType.Refcursor);
                if (checkParram != null && checkParram.Any())
                {
                    //if (query.Parram.Length > 0 && query.Parram !=null)
                    //{
                    //    checkParram.First().Value = query.Parram.First();
                    //}
                }
        ...
        public async Task<string> LoadQueryModule([FromBody] ParramModuleQuery query)
        {
            ...
            try
            {
                ...
                if (ds != null)
                {
                    ...
                    var column = new DataColumn("CheckedCheckBox", typeof(int));
                    column.DefaultValue = 0;
                    var tb = ((DataSet)ds).Tables[0];//.Add(column);
        ...
        [HttpGet]
        [Route("GetAllDefTask")]
        public async Task<ActionResult> GetAllDefTask()
        {
            var outPut = new RestOutput<List<DefTasks>>();
            try
            {
                var m_Client = new CoreController(_configuration);
                var data = m_Client.GetAllDefTask();
                outPut.Data = data;
                return Ok(outPut);
            }
            catch (Exception ex)
        ...
        [HttpGet]
        [Route("GetAllModWorkflow")]
        public async Task<ActionResult> GetAllModWorkflow()
        {
            var outPut = new RestOutput<List<ModWorkflow>>();
            try
            {
                var m_Client = new CoreController(_configuration);
                var data = m_Client.GetAllModWorkflow();
                outPut.Data = data;
                return Ok(outPut);
            }
            catch (Exception ex)
        ...
        [HttpGet]
        [Route("GetAllModTreeView")]
        public async Task<ActionResult> GetAllModTreeView()
        {
            var outPut = new RestOutput<List<ModTreeView>>();
            try
            {
                var m_Client = new CoreController(_configuration);
                var data = m_Client.LoadAllModTreeView();
                outPut.Data = data;
                return Ok(outPut);
            }
            catch (Exception ex)
        ...
        [HttpPost]
        [Route("GetDataTreeviewInfo")]
        public async Task<ActionResult> GetDataTreeviewInfo(ParramModuleQuery query)
        {
            var outPut = new RestOutput<List<TreeviewInfo>>();
            try
            {

                var m_Client = new CoreController(_configuration);
                var data= m_Client.GetDataTreeviewInfo(query.Store, query.Parram);
                outPut.Data = data;
                outPut.ResultCode = 1;
                return Ok(outPut);
            }
