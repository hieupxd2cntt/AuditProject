...
namespace Core.Utils
{
    public static class FieldUtils
    {
        ...
        public static List<ModuleFieldInfo> GetModuleFieldByGridName(string moduleID, string fieldGroup, string fieldName)
        {
            var fieldInfos = (from field in AllCaches.ModuleFieldsInfo
                              where field.ModuleID == moduleID && field.FieldGroup == fieldGroup && field.FieldName.Substring(0,fieldName.Length) == fieldName
                              select field).ToList();            
            return fieldInfos;
        }
