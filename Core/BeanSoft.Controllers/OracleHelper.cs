...
namespace Core
{
    public static class OracleHelper
    {
        ...
        public static void GetStockDapper(string connectionString, out string totaltime)
        {
            totaltime = null;
            using (var conn = new OracleConnection(connectionString))
            {
                try
                {
                    ...
                    string sql = "select * from stockpricehist";
                    var stockprice = conn.Query(sql).ToList();
                    totaltime = string.Format("{0:#,0.000} second(s)", (DateTime.Now - startTime).TotalSeconds);
        ...
        public static void GetStock(string connectionString, out string totaltime)
        {
            totaltime = null;
            using (var conn = new OracleConnection(connectionString))
            {
