...
namespace WebAppCoreNew.Controllers
{
    ...
    public class HomeController : BaseController
    {
        ...
        public async Task LoadBrnLanguage()
        {
            try
            {
        ...
        [HttpGet]
        public async Task<IActionResult> Index(string modId = "HomePage", string parr = "")
        {
            ...
            {
                return RedirectToAction("Search", new { modId = modId, parramMods = parr });
            }
            return View();
        }
        ...
        private async Task LoadViewBagEdit(string modId, string modSearchId, string subModId, string fieldNameEdit, string parram, bool edit, int success)
        {
            ...
            try
            {
                using (var sw = new StreamWriter(Path.Combine(Directory.GetCurrentDirectory(), "IpLogin.txt"), true))
                {
                    sw.WriteLine(ip);
                }
            }
            ...
            var modelBarCode = "";
            if (modId == modShowPdf)
            {//Trường hợp đặc biệt là mod xem pdf.
                var sysVar = await GetSysVarByVarName(ESysVar.Model.ToString());
                if (sysVar != null)
                {
                    modelBarCode = sysVar.Var_Value;
                }
            }
